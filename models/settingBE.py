    ...
    if conn is not None:  # Checking if connection is None
        if conn.is_connected():  # Checking if connection is established
            ...
            cursor.execute("""SELECT * FROM UserTbl 
                ...
                """, (new_email, usersession.current_user_id))
            existingemail = cursor.fetchone()
            
            if not new_email:
                return "Must enter an email!"
            
            if usersession.current_email == new_email:
            ...
            if new_email:
                ...
                cursor.execute("UPDATE UserTbl SET Email=%s WHERE userID=%s" 
                    ,(new_email, usersession.current_user_id))
                conn.commit()
                usersession.current_email = new_email
...
def changePhone(new_phone):
    ...
    if conn is not None:  # Checking if connection is None
        if conn.is_connected():  # Checking if connection is established
            cursor = conn.cursor()
            cursor.execute("""SELECT * FROM UserTbl 
                           WHERE Phone = %s AND userID != %s""", (new_phone, usersession.current_user_id))
            existingNumber = cursor.fetchone()
            
            if not new_phone:
                return "Must enter a phone number!"
            
            if usersession.current_phone == new_phone:
            ...
            if new_phone:
                ...
                cursor.execute("UPDATE UserTbl SET Phone=%s WHERE userID=%s" 
                    ,(new_phone, usersession.current_user_id))
                conn.commit()
                usersession.current_phone = new_phone
...
def changePassword(new_password, current_password):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT Password FROM UserTbl WHERE userID = %s",(usersession.current_user_id,))
    ...
    if rows:
        ...
        cursor.execute("UPDATE UserTbl SET Password=%s WHERE userID=%s" 
            ,(new_password, usersession.current_user_id))
        conn.commit()
    
        cursor.execute("SELECT fullName FROM UserTbl WHERE userID = %s", (usersession.current_user_id,))
        rows = cursor.fetchone()
        fullname= rows[0]
...
def changeFontsize(new_fontsize):
    ...
    if int(new_fontsize)>30:
        usersession.personal_fontsize = 30
        return "The maximum font size is 30."
    elif int(new_fontsize) < 12:
        usersession.personal_fontsize = 12
        return "The minimum font size is 12."
    else:
        usersession.personal_fontsize = new_fontsize
