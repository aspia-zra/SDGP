...
from models.usersession import usersessions
...
class settings(ctk.CTkFrame):
    def __init__(self, main):
        ...
        self.nav = nav.navbar(self, self.main)
        self.nav.grid(row=0, column=0, sticky="ns")

        self.session = usersessions()
    ...
    def createSettings(self):
        ...
        titlelabel = Label(titleFrame, text="Settings", font=TITLE_FONT, bg = BACKGROUND)
        ...
        phoneNumber = Label(phoneFrame, text="Current Number: " + self.session.current_phone)
        ...
        def submit_phone():
            ...
            if "updated" in msg:
                phoneNumber.config(text="Current Number: " + self.session.current_phone)
        ...
        email = Label(emailFrame, text="Current Email: " + self.session.current_email)
        ...
        def submit_email():
            ...
            if "updated" in msg:
                email.config(text="Current Email: " + self.session.current_email)
            elif "Must" in msg or "already" in msg:
                email.config(text="Current Email: " + self.session.current_email)
        ...
        submitpassword.grid(row= 4, column=1,pady=10)
        ...
        fontsize = ctk.CTkLabel(fontsizeframe, text="Current Font Size: " + str(self.session.personal_fontsize))
        fontsize.grid(row=0, column=1, pady=10)

        fontsizelbl = ctk.CTkLabel(fontsizeframe, text="Change Font Size:")
        fontsizelbl.grid(row=1, column=1, pady=10)

        changefontsizeEntry = ctk.CTkEntry(fontsizeframe)
        ...
        def submit_fontsize():
            ...
            if "updated" in msg:
                fontsize.config(text="Current Font Size: " + str(self.session.personal_fontsize))
                
                titlelabel.config(font=("Arial", self.session.personal_fontsize, "bold"))
                phoneNumber.config(font=("Arial", self.session.personal_fontsize))
                changePhoneNumber.config(font=("Arial", self.session.personal_fontsize))
                email.config(font=("Arial", self.session.personal_fontsize))
                emailLabel.config(font=("Arial", self.session.personal_fontsize))
                currentPasswordlbl.config(font=("Arial", self.session.personal_fontsize))
                changePasswordlbl.config(font=("Arial", self.session.personal_fontsize))
                fontsizelbl.config(font=("Arial", self.session.personal_fontsize))
                submitphone.config(font=("Arial", self.session.personal_fontsize))
                submitemail.config(font=("Arial", self.session.personal_fontsize))
                submitpassword.config(font=("Arial", self.session.personal_fontsize))
                submitfontsize.config(font=("Arial", self.session.personal_fontsize))
            
                changefontsizeEntry.delete(0, END)

            self._schedule_result_clear(result_label)

        submitfontsize = ctk.CTkButton(fontsizeframe, text="Update Font Size", command = submit_fontsize)
        submitfontsize.grid(row=3, column=1,pady=10)

        result_label = ctk.CTkLabel(titleFrame, text="")
